...
from context_reliability_testing.workspace import WorkspaceError, WorkspaceManager, apply_condition

from .conftest import init_repo_with_context


def test_workspace_worktree_lifecycle_and_diff_stat(tmp_path: Path) -> None:
    origin = tmp_path / "origin"
    head = init_repo_with_context(origin)
    base = tmp_path / "ws"
